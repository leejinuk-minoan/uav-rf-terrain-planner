...
현재 이 값은 scoring에 반영하지 않는다. Task 033A가 output contract를 정의했고, Task 033B는 PR #85를 통해 `main`에 병합되어 optional diagnostics를 preview JSON, plain text, report에 구현한다. Appendix-table 확장과 field RF validation은 별도 future research 범위다.
...
7. Appendix table 확장, scoring 반영, field RF validation은 각각 별도의 reviewed Task로 분리한다.
