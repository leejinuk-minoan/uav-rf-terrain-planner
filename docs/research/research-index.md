...
현재 이 값은 scoring에 반영하지 않는다. Task 033B는 PR #85를 통해 optional diagnostics를 preview JSON, plain text, report에 구현했고, Task 034B는 PR #91을 통해 기본 11-column table을 보존하는 별도 14-column diagnostic appendix formatter를 `main`에 구현했다. Task 034C는 CLI stdout/file delivery의 단계적 opt-in 계약만 정의하며 Task 034D runtime 구현은 아직 수행되지 않았다. Field RF validation은 별도 future research 범위다.
