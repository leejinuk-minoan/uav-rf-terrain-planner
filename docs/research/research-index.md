...
8. Task 035A는 EPSG:5179 target과 real DEM/DSM adapter를 candidate records 및 rendering-independent actual-geometry features까지 연결하는 Task 035B 계약을 정의했고, Task 035B는 이 engine boundary를 구현했다. MGRS wrapper, rendered map, route, waypoint와 UI는 후속으로 분리한다.
9. Report composition, scoring 반영과 field RF validation은 각각 별도의 reviewed Task로 분리한다.
