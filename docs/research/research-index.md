...
현재 이 값은 scoring에 반영하지 않는다. Preview/report 보조 진단정보로 출력하는 방향은 Task 033A에서 계약을 정의하고, 실제 runtime 구현은 Task 033B에서 별도로 검토한다.
...
6. Dominant obstacle diagnostics의 preview/report 출력은 Task 033B에서 구현을 검토한다.
7. Appendix table 확장과 scoring 반영은 각각 별도의 reviewed Task로 분리한다.
