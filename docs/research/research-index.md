...
현재 이 값은 scoring에 반영하지 않는다. Task 033A가 output contract를 정의했고, Task 033B는 PR #85를 통해 `main`에 병합되어 optional diagnostics를 preview JSON, plain text, report에 구현한다. Task 034A는 기본 appendix table을 변경하지 않는 별도 diagnostic appendix-table 계약만 정의하며, Task 034B runtime 구현은 아직 수행되지 않았다. Field RF validation은 별도 future research 범위다.
...
7. Task 034A는 기본 appendix table의 byte contract를 보존하면서 별도 diagnostic appendix formatter의 계약을 정의한다. Task 034B 구현, report composition, CLI/file output은 아직 승인된 runtime 상태가 아니다.
8. Scoring 반영과 field RF validation은 각각 별도의 reviewed Task로 분리한다.
