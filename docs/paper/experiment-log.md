...
`within_operation_radius == False`이면 Excluded로 분류한다. `dsm_los_score == 0`, `shielding_stability_score == 0`, 또는 `overall_score < 40`이면 Red로 분류한다. `40 <= overall_score < 60`은 Orange, `60 <= overall_score < 75`은 Yellow, `overall_score >= 75`이고 DSM LOS가 clear이면 Green으로 분류한다.
...
Task 009의 route candidate 결과를 기반으로 후속 지도/UI와 논문 분석에 사용할 수 있는 약 500m 간격 waypoint report data structure와 계산 helper를 준비한다.

## 데이터 종류

- synthetic waypoint report only
- actual_drone_operation: false
- actual_link_measurement: false
- real_dem_dsm_loading: false
- map_rendering: false
- route_execution: false
- flight_command_generation: false
- autopilot_integration: false
- 실제 링크상태 데이터: 없음

## 좌표계 및 범위

- WaypointSourcePoint 기반 ordered source point 구조
- WaypointSamplingConfig 기반 약 500m spacing selection
- RouteWaypoint/RouteWaypointReport 기반 offline route reporting point 구조
- AGL 기반 flight_msl_m 계산
- 발진기지 terrain MSL 대비 고도차 계산
- cumulative distance, segment distance, color class, shielding score 기록

## 입력 파라미터

- route_id
- WaypointSourcePoint sequence
- flight_agl_m
- launch_terrain_msl_m
- WaypointSamplingConfig

## 방법

source_points는 cumulative_distance_m 기준 non-decreasing이어야 한다. include_start=True이면 첫 point를 포함한다. spacing_m target 이상이 되는 첫 source point를 waypoint source로 선택한다. include_end=True이면 마지막 point를 포함한다. interpolation은 구현하지 않고 후속 Task로 분리한다.

## 실행 환경

- Cloud: GitHub connector file operations
- Local: Not run in this cloud/GitHub-only context.
- CI: GitHub Actions CI success observed for PR #28 head commit `8c6231ba722aea11aff2f57fc1d82e4f8230e362`

## 실행 명령

- Local commands: Not run in this cloud/GitHub-only context.
- CI commands: GitHub Actions executed install, syntax check, pytest, ruff, and mypy successfully on the checked PR #28 run.

## 결과

- waypoint output scaffold 가능 여부: success on checked PR #28 CI run
- Task 010 CI status: success
- package install in CI: success
- syntax check in CI: success
- pytest in CI: success
- ruff in CI: success
- mypy in CI: success
- 실제 DEM/DSM 기반 waypoint: 미산출
- 지도 렌더링 waypoint layer: 미산출
- 실제 비행경로 실행: 미수행

## 해석

Task 010은 실제 비행 waypoint command가 아니라 후속 지도/UI와 논문 분석에 사용할 offline waypoint report data structure 준비 단계다. 최종 논문에서는 경로별 누적거리, 구간거리, AGL/MSL, 발진기지 대비 고도차, color class, shielding score를 분석·보고용 데이터로 활용할 수 있다.
...
- 실제 비행경로 실행 없음
- 비행명령 생성 없음
- autopilot/control integration 없음
- 지도 렌더링 없음
- Folium/Streamlit UI 없음
- interpolation 고급 구현 없음
- Task 011 UI 구현 없음

## 논문 반영 가능 여부

방법론의 waypoint report model, 500m interval report point sampling, AGL/MSL/발진기지 대비 고도차 산출 구조, route summary table 설계에는 반영 가능하다. 실제 지형 적용 결과, 실제 지도 렌더링 결과, 실제 비행 결과, 실제 링크품질 검증 결과로는 아직 반영할 수 없다.

## GPT Master 검토 메모

PR #28 기준 waypoint가 실제 flight command/autopilot/control field가 아닌 offline route reporting point로만 구현되었는지 확인했다. 실제 DEM/DSM, UI, route execution, RSSI/SINR/packet_loss, autopilot/control field가 포함되지 않았는지도 확인했다. CI success 확인 후 merge 가능하다.

---

# Experiment EXP-20260709-011
...
synthetic input data를 사용하여 candidate score, color classification, route candidate, waypoint report 흐름이 하나의 재현 가능한 offline synthetic scenario output으로 연결되는지 확인한다.

## 데이터 종류

- synthetic end-to-end output only
- synthetic_e2e_output=true
- actual_drone_operation=false
- actual_link_measurement=false
- real_dem_dsm_loading=false
- map_rendering=false
...
- CandidateScore 기반 synthetic candidate records
- ColorClass 기반 candidate color count
- RouteCandidate 기반 synthetic route outputs
- RouteWaypointReport 기반 waypoint report outputs
- selected route summary
- console/example summary string

## 입력 파라미터

- synthetic candidate score component values
- synthetic RouteCell sequence
- synthetic WaypointSourcePoint sequence
- scenario_name

## 방법

Task 007 scoring helper로 CandidateScore를 만들고, Task 008 classification helper로 ColorClass를 산출한다. Task 009 routing helper로 세 가지 route candidate를 만들고, Task 010 waypoint helper로 각 route의 waypoint report를 만든다. selected_route_id는 select_lowest_cost_route() 결과를 사용한다. summary는 candidate color counts와 selected route/waypoint metrics를 포함한다.
...
- example script: 구현
- 실제 DEM/DSM 기반 scenario output: 미산출
- 지도 렌더링 output: 미산출
- 실제 비행경로 실행: 미수행

## 해석

Task 011은 실제 지도 결과나 실제 지형 적용 결과가 아니라, 지금까지 구현한 순수 Python 모듈들이 synthetic data 기준으로 candidate-to-waypoint 흐름으로 연결되는지 확인하는 output scaffold 단계다. 논문에서는 실제 지형 적용 전 재현 가능한 synthetic example과 pipeline consistency check로 활용할 수 있다.
...
- 지도 렌더링 없음
- Folium/Streamlit UI 없음
- 실제 지형 기반 scenario output 없음
- Task 012 구현 없음

## 논문 반영 가능 여부

방법론의 candidate-to-waypoint synthetic report, 모듈 연결성 확인, candidate color count summary, selected route summary, 재현 가능한 예제 출력에는 반영 가능하다. 실제 지형 적용 결과, 실제 지도 렌더링 결과, 실제 비행 결과, 실제 링크품질 검증 결과로는 반영할 수 없다.
...
Task 011의 SyntheticEndToEndScenario 결과를 후속 지도/UI가 사용할 수 있는 map-ready data structure로 변환한다.

## 데이터 종류

- synthetic map output only
- synthetic_map_output=true
- actual_drone_operation=false
- actual_link_measurement=false
- real_dem_dsm_loading=false
- map_rendering=false
...
- CandidateCellMapFeature 기반 candidate cell feature records
- RouteMapFeature 기반 route feature records
- WaypointMapFeature 기반 waypoint feature records
- MapStyle 기반 rendering-independent style metadata
- MapOutputPackage 기반 map-ready output package
- 실제 지도 좌표계 변환 없음
- 실제 DEM/DSM 기반 map output 없음

## 입력 파라미터

- SyntheticEndToEndScenario
- SyntheticCandidateRecord sequence
- SyntheticRouteOutput sequence
- RouteWaypointReport waypoints
- selected_route_id

## 방법

Task 011의 synthetic scenario output을 입력으로 받아 candidate cell, route, waypoint feature records를 생성한다. 각 feature에는 후속 UI가 참고할 style metadata를 포함하되, 실제 지도 렌더링, tile/layer 생성, Folium/Streamlit 호출은 수행하지 않는다.

## 실행 환경

- Cloud: GitHub connector file operations
- Local: Not run in this cloud/GitHub-only context.
- CI: GitHub Actions CI success observed for PR #32 head commit `3d60e844db8b7856969b88ea559344d1d82a9908`

## 실행 명령

- Local commands: Not run in this cloud/GitHub-only context.
- CI commands: GitHub Actions executed install, syntax check, pytest, ruff, and mypy successfully on the checked PR #32 run.

## 결과

- map-ready output data scaffold 가능 여부: success on checked PR #32 CI run
- Task 012 CI status: success
- package install in CI: success
- syntax check in CI: success
- pytest in CI: success
- ruff in CI: success
- mypy in CI: success
- candidate cell feature records: 구현
- route feature records: 구현
- waypoint feature records: 구현
- MapOutputPackage summary: 구현
- example script: 구현
- 실제 DEM/DSM 기반 map output: 미산출
- 지도 렌더링 output: 미산출
- 실제 비행경로 실행: 미수행

## 해석

Task 012는 실제 지도 화면을 생성하는 단계가 아니라, 후속 UI/map rendering 단계가 사용할 수 있는 rendering-independent feature data package를 준비하는 단계다. 논문에서는 후보 cell, route, waypoint feature 수, color class 분포, red/orange waypoint feature count, selected route id 등을 시스템 산출 데이터 구조로 설명할 수 있다.
...
- 지도 렌더링 없음
- Folium/Streamlit UI 없음
- 실제 지형 기반 map output 없음
- Task 013 구현 없음

## 논문 반영 가능 여부

방법론의 map-ready data package, candidate/route/waypoint feature records, rendering-independent style metadata, system output table 설계에는 반영 가능하다. 실제 지도 렌더링 결과, 실제 DEM/DSM 적용 결과, 실제 비행 결과, 실제 링크품질 검증 결과로는 반영할 수 없다.

## GPT Master 검토 메모

PR #32는 코드 기준 Task 012 acceptance criteria의 핵심 구현을 충족한다. Map output은 실제 지도 렌더링이 아니라 SyntheticEndToEndScenario를 후속 UI/지도 구현이 읽을 수 있는 map-ready data package로 변환하는 offline output scaffold로 구현되었는지 확인했다. 실제 DEM/DSM loading, 지도 렌더링, route execution, RSSI/SINR/packet_loss, autopilot/control field가 포함되지 않았는지도 확인했다. CI success 확인 후 merge 가능하다.
