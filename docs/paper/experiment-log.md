...
- profile extraction 가능 여부: PR/CI 확인 전
- Task 004 scaffold CI status: PR 생성 후 확인 필요
...
Task 004는 실제 실험 결과가 아니라 synthetic terrain profile extraction 준비 단계다. 논문 결과 장에는 아직 반영하지 않는다.
...
PR 생성 후 profile sample 구조, start/end 포함, 거리 필드, DEM/DSM/DSM-DEM delta, out-of-bounds error, CI 결과를 기준으로 편입 여부를 판단한다.
