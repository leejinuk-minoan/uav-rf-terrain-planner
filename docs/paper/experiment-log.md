...
- waypoint output scaffold 가능 여부: PR/CI 확인 전
- Task 010 CI status: PR 생성 후 확인 필요
- package install in CI: PR 생성 후 확인 필요
- syntax check in CI: PR 생성 후 확인 필요
- pytest in CI: PR 생성 후 확인 필요
- ruff in CI: PR 생성 후 확인 필요
...
PR 생성 후 waypoint가 실제 flight command/autopilot/control field가 아닌 offline route reporting point로만 구현되었는지 확인한다. 실제 DEM/DSM, UI, route execution, RSSI/SINR/packet_loss, autopilot/control field가 포함되지 않았는지도 확인해야 한다.
