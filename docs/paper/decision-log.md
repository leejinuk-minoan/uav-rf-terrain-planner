...
PR #30 생성 후 synthetic end-to-end output이 실제 지도/UI 구현이 아니라 candidate scoring, color classification, route candidate, waypoint report를 synthetic data로 연결하는 offline scenario output scaffold로만 구현되었는지 확인해야 한다. 실제 DEM/DSM loading, 지도 렌더링, route execution, RSSI/SINR/packet_loss, autopilot/control field가 포함되지 않았는지도 확인해야 한다. CI success 확인 후 merge 여부를 판단한다.
