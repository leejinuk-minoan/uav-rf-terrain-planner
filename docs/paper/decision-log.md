...
Task 012는 실제 지도/UI 구현이 아니라 SyntheticEndToEndScenario를 후속 UI/지도 구현이 읽을 수 있는 map-ready data package로 변환하는 offline output scaffold로 정의한다.

## 배경

Task 011에서 candidate scoring, color classification, route candidate, waypoint report가 synthetic data 기준으로 연결되었으므로, 후속 UI/map rendering 단계에서 사용할 feature data package가 필요하다.

## 대안

1. Task 012에서 rendering-independent map-ready data structure만 구현
2. Task 012에서 Folium/Streamlit 지도 렌더링까지 구현
3. Task 012에서 실제 DEM/DSM 기반 map output까지 구현
4. Task 012에서 실제 비행경로 또는 링크품질 검증까지 포함

## 선택 이유

1안을 선택한다. Map-ready data package와 실제 지도 렌더링, 실제 DEM/DSM 연결, UI 검증은 책임이 다른 단계이므로 분리해야 한다. 실제 비행경로 실행, autopilot/control integration, 링크품질 검증은 본 Task 범위 밖이다.
...
- `src/uav_rf_terrain/__init__.py`
- 향후 UI/map rendering 모듈

## 논문 반영 위치

- 방법론: map-ready output data package
- 재현성: candidate/route/waypoint feature records, style metadata, summary counts
- 결과/민감도 분석 준비: candidate feature color distribution, waypoint red/orange count, selected route id
- 한계: 실제 DEM/DSM 및 지도 렌더링 전 단계

## 검증 필요사항

- MapOutputError, MapFeatureType, MapStyle validation
- CandidateCellMapFeature, RouteMapFeature, WaypointMapFeature validation
- MapOutputPackage validation
- SyntheticEndToEndScenario → MapOutputPackage 연결 확인
- candidate / route / waypoint feature records 생성 확인
- summary key 및 color count 확인
- map-ready data, not rendered map 문구 확인
- RSSI/SINR/packet_loss 필드 부재 확인
- flight command/autopilot/control field 부재 확인
- map/GIS dependency 부재 확인
- 기존 experiment/decision/pr-review logs 삭제·축약 여부 확인
