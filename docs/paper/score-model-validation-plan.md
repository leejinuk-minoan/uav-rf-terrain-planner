...
차폐안정성 점수는 DSM 기반 LOS 점수와 DSM 기반 Fresnel 여유 점수만으로 계산한다.

```text
차폐안정성 점수 = DSM 기반 LOS 점수 × 0.40
                 + DSM 기반 Fresnel 여유 점수 × 0.60
```
...
### 2.1 LOS/Fresnel 비율 선정 근거

본 연구의 기본 가중치는 `LOS 0.40 : Fresnel 0.60`으로 둔다.

이유는 다음이다.

1. LOS는 통신 경로의 중심선 차단 여부를 판단하는 필수 조건이다.
2. 그러나 LOS는 대체로 차단/비차단 성격이 강해 후보지 간 세부 차이를 충분히 구분하기 어렵다.
3. Fresnel 평가는 LOS 중심선 주변의 전파 통로 여유를 평가하므로, LOS가 열려 있는 후보지 사이의 품질 차이를 더 정밀하게 구분한다.
4. 무선 링크 설계에서는 가시선이 열려 있어도 제1 Fresnel Zone 침범이 신호 약화로 이어질 수 있으므로 Fresnel 여유를 LOS보다 약간 더 크게 반영한다.
5. 다만 LOS 차단 후보가 Fresnel 점수로 보상되는 것을 막기 위해, 구현 단계에서는 LOS 차단 또는 심각한 침범 후보에 대해 별도 cap 또는 제외 규칙을 둘 수 있다.
...
```text
if DSM_LOS_score < 50:
    shielding_stability_score = min(shielding_stability_score, 40)
```

즉, 기본 가중치는 Fresnel을 더 크게 두되, LOS는 필수 조건으로 작동하도록 설계한다.
...
본 연구에서는 별도의 표면장애물 복잡도 보정점수를 기본 점수식에 포함하지 않는다.

제외 이유는 다음과 같다.

1. DSM 기반 LOS 점수는 건물·수목·구조물이 중심선을 직접 차단하는 효과를 이미 반영한다.
2. DSM 기반 Fresnel 점수는 건물·수목·구조물이 제1 Fresnel Zone을 침범하는 효과를 이미 반영한다.
3. 표면장애물 복잡도를 별도 점수로 추가하면 같은 DSM 장애물을 LOS/Fresnel과 다시 중복 감점할 위험이 있다.
4. 실제 multipath, scattering, foliage penetration, material loss는 DSM만으로 직접 산출하기 어렵고, RSSI/SINR/packet loss 또는 별도 전파모델이 필요하다.
5. 본 연구는 실제 링크 측정값 없이 수행되는 오프라인 차폐위험 proxy 연구이므로, 검증되지 않은 추가 보정항을 넣지 않는다.

따라서 본 연구의 기본 점수식은 처음부터 다음 두 항목만 고려한 것으로 정의한다.

```text
DSM 기반 LOS 점수
DSM 기반 Fresnel 여유 점수
```

---

## 5. AGL 운용고도와 LOS/Fresnel 기하 기준
...
| 오프라인 산출값만 사용 | DEM/DSM, 좌표, 거리, 고도, DSM 기반 LOS, DSM 기반 Fresnel proxy만 사용한다. |
...
| S4 DEM 평탄 + DSM 수목 | 수목 표면고도 존재 | Fresnel 점수 감점 |
...
| 케이스 | DSM LOS | DSM Fresnel | 목적 |
|---|---:|---:|---|
| S1 | 0.50 | 0.50 | 동등 기준 |
| S2 | 0.40 | 0.60 | 기본 권장 기준 |
...
## 14. Cloud Execution Agent 구현 지시 기준
...
4. 표면장애물 복잡도 보정점수는 구현하지 않는다.
5. DEM-only 계산은 fallback 또는 비교실험으로 분리한다.
